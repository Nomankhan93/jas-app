// src/routes/admin/programs/health.tsx
import {
  Link,
  Outlet,
  createFileRoute,
...
function AdminHealthRoute() {
  ...
  if (normalizedPathname === '/admin/programs/health') {
    return <AdminHealthApplicationsPage />
  }
        ...
        <div className="mx-auto max-w-7xl space-y-8">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="grid gap-4 xl:grid-cols-[1fr_200px_200px_200px_200px_auto]">
              ...
              <SelectBox value={emergencyFilter} onChange={setEmergencyFilter}>
          ...
          {loading ? (
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-sm">
              ...
              <p className="mx-auto mt-3 max-w-2xl text-slate-600">
                Selected filters ya search ke according koi health case nahi mila.
              </p>
            </div>
          ) : (
...
async function ensureHealthAdminAccess() {
  ...
  if (userError || !user) {
    return { ok: false, message: 'Health admin panel dekhne ke liye pehle login karen.' }
  }
  ...
  if (rolesError) {
    return { ok: false, message: rolesError.message }
  }

  const roleNames = ((roles || []) as UserRoleRow[]).map((item) => item.role)
  const hasGlobalAccess = roleNames.some((role) =>
    ['admin', 'super_admin', 'health_admin'].includes(role),
  )

  if (hasGlobalAccess) return { ok: true, message: '' }

  const { data: assignments, error: assignmentError } = await supabase
    .from('program_admin_assignments')
    .select('id, can_view')
    .eq('user_id', user.id)
    .eq('program_key', 'health')
    .eq('can_view', true)
  ...
  if (((assignments || []) as AssignmentRow[]).length > 0) {
...
function exportHealthCsv(items: HealthApplicationListItem[]) {
  ...
  link.href = url
  link.download = `jas-health-applications-${new Date()
    .toISOString()
    .slice(0, 10)}.csv`
