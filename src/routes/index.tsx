...
const portalStats = [
  ...
  { label: 'Application Fee', value: `${formatMembershipMoney(MEMBERSHIP_BASE_FEE)} + charges` },
]
    ...
    <section className="soft-panel animate-fade-up relative overflow-hidden rounded-[2rem] border-[#e8e0d1] bg-[linear-gradient(135deg,#fffdf8_0%,#f7f1e6_50%,#edf4ee_100%)] p-[clamp(1.5rem,4vw,3.5rem)] shadow-[0_30px_80px_rgba(11,42,29,0.10)]">
      ...
      <div className="relative z-10 grid items-center gap-12 lg:grid-cols-[minmax(0,1fr)_430px]">
        <div>
          ...
          <div className="mt-6 max-w-2xl rounded-[1.35rem] border border-amber-200 bg-amber-50/80 p-4 text-sm leading-6 text-amber-900 shadow-sm"><p className="font-black">Membership Application Fee: {formatMembershipMoney(MEMBERSHIP_BASE_FEE)} + {MEMBERSHIP_PROCESSING_LABEL}.</p><p className="mt-1 text-amber-800">{getMembershipFeeSubtext()}</p></div>
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{portalStats.map((item, index) => <div key={item.label} className={`soft-panel animate-fade-up ${getDelayClass(index)} rounded-[1.1rem] border-white/70 bg-white/72 px-4 py-3 shadow-sm backdrop-blur`}><p className="m-0 text-[0.66rem] font-extrabold uppercase tracking-[0.18em] text-stone-400">{item.label}</p><p className="mt-1 text-sm font-black text-stone-950">{item.value}</p></div>)}</div>
        </div>
