...
export function CmsPublicPage({ slug }: CmsPublicPageProps) {
  ...
  const subtitle = page?.subtitle || config.fallbackSubtitle
  const content = page?.content || config.fallbackContent
      ...
      <div className="page-wrap space-y-8">
        <section className="soft-panel relative overflow-hidden rounded-[2rem] border border-[#e8e0d1] bg-[linear-gradient(135deg,#fffdf8_0%,#f7f1e6_54%,#edf4ee_100%)] p-[clamp(1.5rem,4vw,3.6rem)] shadow-[0_28px_80px_rgba(11,42,29,0.10)]">
          ...
          <div className="relative z-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-center">
            <div>
              <p className="section-eyebrow mb-4">{config.eyebrow}</p>
            ...
            <div className="rounded-[1.8rem] border border-emerald-900/10 bg-white/75 p-6 shadow-sm backdrop-blur">
              ...
              <h2 className="mt-5 text-xl font-black text-stone-950">
                Official JAS Content
              </h2>
              <p className="mt-2 text-sm leading-7 text-stone-600">
                These organization pages are managed from the CMS so future
                updates can be published without changing code.
              </p>
              <div className="mt-4 rounded-2xl bg-emerald-50 px-4 py-3 text-xs font-black uppercase tracking-[0.14em] text-emerald-900">
                {loading ? 'Loading...' : page ? 'Published from CMS' : 'Fallback content'}
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <article className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-sm sm:p-8">
            <div className="prose-content space-y-5">
              {blocks.map((block, index) => {
                ...
                if (isList) {
                  return (
                    <ul key={index} className="list-disc space-y-2 pl-6 text-[1rem] font-semibold leading-8 text-slate-700">
                      {lines.map((line) => (
          ...
          <aside className="space-y-4">
            <InfoCard title="Membership" to="/signup" text="Join JAS as a verified member." />
            <InfoCard title="Programs" to="/programs/education" text="Explore education, health, welfare and employment support." />
            <InfoCard title="Donors" to="/donors" text="View member-only donor leaderboard." />
          </aside>
        </section>
      </div>
    </main>
