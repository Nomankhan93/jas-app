  ...
  return (
    <div className="relative">
      ...
      {open ? (
        <div
          onClick={onClose}
          className="absolute right-0 top-full z-[60] mt-4 w-[420px] rounded-3xl border border-slate-200 bg-white p-3 shadow-[0_24px_70px_rgba(15,23,42,0.18)]"
        >
...
function PublicPageDropdownItem({ item, active }: { item: PublicPageItem; active: boolean }) {
  return (
    <Link
      to={item.to}
      className={`group flex items-start gap-3 rounded-2xl p-3 no-underline transition ${active ? 'bg-amber-50 text-amber-900' : 'text-slate-700 hover:bg-slate-50 hover:text-amber-900'}`}
    >
